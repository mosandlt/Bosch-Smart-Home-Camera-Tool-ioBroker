 ...
 */
function parseFormFields(html, pageUrl) {
...
async function loginWithCredentials(httpClient, username, password) {
    ...
    const jarClient = (0, axios_cookiejar_support_1.wrapper)(axios_1.default.create({
    ...
    let emailHtml;
    try {
        const emailPageResp = await jarClient.get(authUrl, {
        ...
        if (axios_1.default.isAxiosError(err)) {
            ...
            if (status === 400) {
                throw new LoginFlowError(`Auth page returned HTTP 400: ${JSON.stringify(err.response?.data ?? "").slice(0, 200)}`);
            }
    ...
    if (!emailForm.action || !emailForm.csrf) {
        throw new LoginFlowError("Email page: could not find form action or CSRF token — page structure changed");
    }
    // ── Step 4: POST email ───────────────────────────────────────────────────
    ...
    try {
        ...
        if (axios_1.default.isAxiosError(err)) {
            ...
            throw new LoginFlowError(`Network error during email POST: ${err.message}`);
        }
    ...
    if (isStillOnEmailPage) {
        ...
        if (emailPreFilled && detectCaptcha(passwordHtml)) {
            throw new MfaRequiredError("hCaptcha required on email page — headless programmatic login is blocked by " +
        ...
        throw new InvalidCredentialsError("Email rejected by Bosch SingleKey ID — check your email address");
    }
    ...
    if (!passwordForm.action || !passwordForm.csrf) {
        throw new LoginFlowError("Password page: could not find form action or CSRF token — page structure changed");
    }
    ...
    try {
        ...
        if (axios_1.default.isAxiosError(err)) {
            ...
            throw new LoginFlowError(`Network error during password POST: ${err.message}`);
        }
